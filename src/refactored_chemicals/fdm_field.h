#ifndef MICROBESIMULATOR_REFACTORED_FDM_FIELD_H
#define MICROBESIMULATOR_REFACTORED_FDM_FIELD_H

#include "./field_base.h"

#include "../geometry/geometry.h" // also contains BoundaryCondition enum
...
public:
	...
	// FDM_Field(const Point<dim>& lower, 
...
// template<int dim, typename Number>
// FDM_Field<dim, Number>::FDM_Field(const Point<dim>& lower, const Point<dim>& upper,
...
template<int dim, typename Number>
void 
FDM_Field<dim, Number>::init(const Point<dim>& lower, const Point<dim>& upper,
...
template<int dim, typename Number>
double 
FDM_Field<dim,Number>::getInverseCellWidthSquared(unsigned int dimension) const
{
...
template<>
double 
FDM_Field<1, double>::at_bc(int i) const
{
...
template<>
double 
FDM_Field<2, double>::at_bc(int /*i*/) const
{
...
template<>
double 
FDM_Field<3, double>::at_bc(int /*i*/) const
{
	std::cout << "not implemented" << std::endl;
	assert(false);
}
...
template<>
double 
FDM_Field<1, double>::at_bc(int /*i*/, int /*j*/) const
{
	std::cout << "not implemented" << std::endl;
	assert(false);
}
...
template<>
double 
FDM_Field<2, double>::at_bc(int i, int j) const
{
	if( (i == -1) && (boundary_conditions[0] == BoundaryCondition::WRAP))
		i = (int)(this->n_cells)[0]-1;
	else if( (i == -1) && (boundary_conditions[0] == BoundaryCondition::REFLECT))
...
template<>
double 
FDM_Field<3, double>::at_bc(int /*i*/, int /*j*/) const
{
	std::cout << "not implemented" << std::endl;
	assert(false);
}
...
template<>
double
FDM_Field<1, double>::at_bc(int /*i*/, int /*j*/, int /*k*/) const
{
	std::cout << "not implemented" << std::endl;
	assert(false);
}
...
template<>
double
FDM_Field<2, double>::at_bc(int /*i*/, int /*j*/, int /*k*/) const
{
	std::cout << "not implemented" << std::endl;
	assert(false);
}
...
template<>
double
FDM_Field<3, double>::at_bc(int i, int j, int k) const
{
	if( (i == -1) && (boundary_conditions[0] == BoundaryCondition::WRAP))
		i = (this->n_cells)[0]-1;
	else if( (i == -1) && (boundary_conditions[0] == BoundaryCondition::REFLECT))
...
template<int dim, typename Number>
void
FDM_Field<dim, Number>::print(std::ostream& out) const
{
	for(unsigned int i = 0; i < this->field.size(); ++i)
		out << this->field[i] << " ";
	out << std::endl;
